...
public class DirectedGraph {
	...
	public boolean isAcyclic() {
		...
		boolean[] pop = new boolean[this.nodes.size()];
		
		int V = this.nodes.size();
		for(int v=0; v<V; v++)//visit every vertices.
		{
			if( isCyclic(v, visited, pop) )//check if it is a cyclic.
			{
				return false;
			}
		}
	...
	private boolean isCyclic(int v, boolean[] visited, boolean[] pop) {
		
		if( pop[v] )//if it's already poped.
		{
			return true;
		}
		if( visited[v] )//if it's already visited.
		{
			return false;
		}
		visited[v] = true;//A current vertex
		pop[v] = true;//A current vertex is poped.
		
		Iterator<DirectedGraphNode> iterator = this.nodes.get(v).outgoingNodes.iterator();
		//iterate through vertices which is connected to v.
		while( iterator.hasNext() )
		{
			DirectedGraphNode dir = (DirectedGraphNode) iterator.next();
			//recursively visit vertices to see if there is a cyclic.
			if( isCyclic(dir.getData(), visited, pop) )
			{
				return true;
			}
		}
		pop[v] = false;//put v back to the queue.
		return false;
	}
