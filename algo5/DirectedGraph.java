...
public class DirectedGraph {
	...
	private int vCover(DirectedGraphNode root) {
		//YOUR CODE HERE
		int opt1=0, opt2=0, size=0;
		//if node is a leaf, set it to 0.
		if(root.getCoverSize() == null) 
		{
			//Counting vCover including n vertex
			Iterator<DirectedGraphNode> iterator = root.outgoingNodes.iterator();
			while( iterator.hasNext() ) 
			{
				DirectedGraphNode dir = (DirectedGraphNode) iterator.next();
				opt1 = opt1 + vCover(dir);
			}
			//Counting vCover excluding n vertex
			Iterator<DirectedGraphNode> iterator2 = root.outgoingNodes.iterator();
			size = root.getOutDegree();
			while( iterator2.hasNext() ) 
			{
				DirectedGraphNode dir2 = (DirectedGraphNode) iterator2.next();
				Iterator<DirectedGraphNode> iterator3 = dir2.outgoingNodes.iterator();
				//going through grand children
				while( iterator3.hasNext() )
				{
					DirectedGraphNode dir4 = (DirectedGraphNode) iterator3.next();
					opt2 = opt2 + vCover(dir4);
				}
			}
			opt2 = opt2 + size;
			//Choose minimum vertex cover of the two cases so far.
			root.setCoverSize( Math.min(1+opt1, opt2) );
		}//base case
	 ...
	 * returned by vCover(). Note that this method should first call the vCover
	...
	public void printCover() {
		//YOUR CODE HERE
		int vCover = getMinVertexCover();
		if( vCover > 0 ) {
			
			for(DirectedGraphNode node: this.nodes) {
				//if it is middle node, and a part of minimum vertex cover
				if( node.getCoverSize()>0 &&
						node.getCoverSize() <= node.getOutDegree() ) {
